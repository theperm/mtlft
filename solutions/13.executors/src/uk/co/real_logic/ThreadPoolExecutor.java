    ...
    implements OverflowExecutor
{
    ...
    /**
     * Create a new {@link java.util.concurrent.Executor} with bounded internal queue size.
     *
    ...
    /**
     * Create a new {@link java.util.concurrent.Executor} with a bounded internal queue size.
     *
     * @param threadPoolSize number of threads in the execution pool
     * @param queueCapacity for the bounded internal queue
    ...
    private class MonitorRunner implements Runnable
    {
        ...
        {
            for (int i = 0, limit = parkedWorkers.length(); i < limit; i++)
            {
                final Thread parkedThread = parkedWorkers.getAndSet(i, null);
                if (null != parkedThread)
                {
